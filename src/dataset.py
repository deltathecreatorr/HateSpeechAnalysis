...
def getRedditDataframe() -> pd.DataFrame:
    ...
    """
    df = pd.read_csv('dataset/reddit_opinion_PSE_ISR.csv', chunksize = 1000, usecols = ["author_name", "post_id", "comment_id", "self_text", "created_time", "user_total_karma", "user_account_created_time"])
    return df
...
def getTwitterDataframe(filepath: str, search_query: str) -> None:
    ...
    async def main():
        await client.login(
            ...
            auth_info_2 = os.getenv("PASSWORD"),
